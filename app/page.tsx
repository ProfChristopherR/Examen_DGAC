...
import { BookOpen, ClipboardCheck, ArrowRight, User, Mail, Shield, Zap, Globe, ChevronRight, HardDrive, History, Download, Upload } from 'lucide-react'
import { useRouter } from 'next/navigation'
import { toast } from 'sonner'
import { getUser, setUser, exportData, importData } from '@/lib/storage'
...
function GoogleSignInButton({ onSuccess }: { onSuccess: (profile: { sub: string; name: string; email: string; picture: string }) => void }) {
      ...
      callback: (response: any) => {
        if (response?.credential) {
          try {
            ...
            toast.error('Error al procesar sesión de Google')
          }
        }
      },
    })
...
export default function Home() {
  ...
  const handleRegister = async () => {
    ...
    setUser({ id: userId, name: name.trim(), email: email.trim() })
    router.push('/mode-select')
    setLoading(false)
  }

  const handleGoogleSuccess = useCallback((profile: { sub: string; name: string; email: string; picture: string }) => {
    setUser({
      id: 'g_' + profile.sub,
      name: profile.name,
      email: profile.email,
  ...
  const handleImport = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (ev) => {
      ...
      if (result.success) {
        ...
        // Redirect if user restored
        const user = getUser()
        if (user) {
          setTimeout(() => router.push('/mode-select'), 800)
        }
      } else {
            ...
            >
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-center mb-14">
                <div className="text-center lg:text-left order-2 lg:order-1">
                  ...
                  >
                    Gratuito • Sin registro complejo • Historial local en tu navegador
                  </motion.p>
                </div>
              ...
              <div className="rounded-3xl bg-white/[0.02] border border-white/[0.08] backdrop-blur-xl p-8 sm:p-10">
                ...
                <div className="mb-6">
                  ...
                  <p className="text-[10px] text-center text-slate-600 mt-2">
                    Opcional. Tus datos se guardan solo en tu navegador.
                  </p>
                </div>
